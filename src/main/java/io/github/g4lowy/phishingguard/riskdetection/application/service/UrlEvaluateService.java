...
class UrlEvaluateService {
    ...
    private Mono<Risk> evaluateUrl(String raw) {
        ...
        return cache.getUrl(urlKey)
                ...
                .switchIfEmpty(heuristicOrApi(normalized)
                        .flatMap(risk -> cache.putUrl(urlKey, risk)
                                .then(cache.putETldPlusOne(e2ld, risk))
                                .thenReturn(risk)
                        )
                );
    }
    ...
    private Mono<Risk> heuristicOrApi(String url) {
        ...
        boolean suspiciousUrl = urlRiskCheckerStrategies
                .stream()
                .anyMatch(strategy -> strategy.breaksRule(url));
